...
import {getContentList, postContentEdit} from "../common/api";
...
const tableList = () => {
  ...
  getContentList(searchFormData).then((res) => {
    total.value = res.data.total
    tableData.value = res.data.list
    scene.info = res.data.scene
  })
}
...
const onEdit = () => {
  ...
  postContentEdit(rowInfo.row).then(res => {
...
<template>
  <el-container class="my-table-container">
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="" label="场景">
        <template #default="scope">
          {{ scene.info[scope.row.scene].desc }}
        </template>
      </el-table-column>
  ...
  <el-dialog v-model="showEditDialog" title="编辑">
    <el-form label-width="5rem">
      <el-form-item label="标题:">
        <el-input v-model="rowInfo.row.name" placeholder="Please input name"/>
      </el-form-item>
      <el-form-item label="备注:">
        <el-input
            v-model="rowInfo.row.content"
            :rows="2"
            type="textarea"
