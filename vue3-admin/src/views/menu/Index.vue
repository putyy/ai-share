...
import MenuEdit from '../../components/user/MenuEdit.vue'
...
const tableList = () => {
  ...
  getMenuList(searchFormData).then((res) => {
...
<template>
  <el-container class="my-table-container">
    ...
    <el-table :data="tableData" height="70vh" style="width: 100%">
      ...
      <el-table-column prop="menu_group.name" label="分类"/>
      <el-table-column prop="name" label="菜单名"/>
      <el-table-column prop="use_vip" label="使用权限">
        <template #default="scope">
          <el-row>
            {{vipConfig[scope.row.use_vip].Name }}
          </el-row>
        </template>
      </el-table-column>
  ...
  <MenuEdit :rowInfo="rowInfo" @changeRow="onChangeRow"/>
