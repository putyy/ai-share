...
func ShareResourceTypeList(c *gin.Context) {
	data := make(map[string]interface{})
	var resList []models.ShareResourceType
	model := models.Db()
	...
	if total == 0 {
		model.Model(models.ShareResourceType{}).Count(&total)
	}
	...
	data["list"] = resList
	data["m_mark"] = "share-resource-type"
	api.ResponseSuccess(c, data)
}

func ShareResourceTypeEdit(c *gin.Context) {
	var formData form.ShareResourceTypeAdminForm
	if err1 := c.ShouldBind(&formData); err1 != nil {
		api.ResponseError(c, "参数错误", err1.Error())
		return
	}

	model := models.ShareResourceType{
		Name:     formData.Name,
		Sort:     formData.Sort,
